...
function TaxiShare() {
	...
	var icon = new H.map.DomIcon("<div>&#x1F695</div>");
	var marker;		
	...
	console.log("Getting Address data" + data);
		address = data.data[0].offer_address;
		clearMarkersFromMap();
		addMarkerToMap(address);
	};
	alert("Searching for taxi offer. Please allow 10 seconds");
    ...
    function register(oucu) {
        ...
        $.ajax(url, { type: "POST", data: { oucu: oucu, type: 0}, success: onSuccess });
    }
	...
	function onSuccess(obj) {
            ...
            // Inform the user what happened
            if (obj.status == "success") {
                alert("User " + oucu + address + startTime + endTime +  " have been successfully offered.");
            } else if (obj.message) {
                alert(obj.message);
            } else {
                alert("Invalid data: " + oucu + address + startTime + endTime);
            }
        }

        // Post the data to offer with the Taxi Sharing API
	...
	function onSuccess(obj) {
            ...
            // Inform the user what happened
            if (obj.status == "success") {
                alert("User " + oucu + address + startTime + " have been successfully requested.");
            } else if (obj.message) {
                alert(obj.message);
            } else {
                alert("Invalid data: " + oucu + address + startTime);
            }
        }
  ...
  function payWithGoogle(share, licence) {
	...
	let request = {
    		merchantId: 'taxiTaxis', 
    		purpose: 'Payment for Taxi ' + licence,
    		amount: 20 / share,
    		countryCode: "GB",
    		currencyCode: "GBP"
	};
	
	checkForApplePayOrGooglePay();
	...
	var url = 'processor sample';
	//Post request to merchant.

	//on Success response data
	request.gateway = 'sample'; 
	request.merchantId = 'TaxiTaxis'; 
	request.gpMerchantName = 'Taxi Share'; 
	request.gpMerchantId = 'OU'; 
    ...
    this.offerTaxi = function () {
        ...
        // TODO adjust the following to get the required data from your HTML view
        var oucu = getInputValue("oucu", "user1");
        var address = getInputValue("address", "Milton Keynes Central Station");
        var startTime = getInputValue("startTime", defaultStartTime); // eg. 2020:12:18:14:38
        var hours = getInputValue("endTime", "1"); // duration in hours
    ...
    this.requestTaxi = function () {
        // TODO adjust the following to get the required data from your HTML view
        var oucu = getInputValue("oucu", "user1");
        var address = getInputValue("address", "Open University, Milton Keynes");
   ...
   this.payTaxi = function () {
	// Call payWithGoogle function
	var share = getInputValue("shareCount", 1);
	var licence = getInputValue("licenceNum", "000");
	payWithGoogle(share, licence);
	};
}
